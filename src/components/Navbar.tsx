...
export default function Navbar() {
  ...
  return (
    <nav className="sticky top-0 z-50 w-full bg-brand-bg/80 backdrop-blur-md border-b border-white/5" ref={menuRef}>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16">
          ...
          <div className="flex md:hidden items-center gap-2">
            <button onClick={toggleMobileMenu} className="p-2 text-neutral-400 hover:text-white">
      ...
      {isMobileMenuOpen && (
        <div className="md:hidden bg-brand-bg border-b border-white/5 p-4 space-y-2">
          {navLinks.map((link) => {
            const Icon = link.icon;
            return (
              <Link
                key={link.path}
                to={link.path}
                className="flex items-center gap-3 px-4 py-3 rounded-lg text-neutral-300 hover:bg-white/5 hover:text-white"
          ...
          {user?.isAdmin && adminLinks.map((link) => {
            ...
            return (
              <Link
                key={link.path}
                to={link.path}
                className="flex items-center gap-3 px-4 py-3 rounded-lg text-brand-secondary hover:bg-white/5"
                onClick={closeMobileMenu}
              >
                <Icon className="w-5 h-5" />
                {link.label}
          ...
          <div className="pt-2 border-t border-white/5 flex items-center justify-between">
            <button
              onClick={toggleLanguage}
              className="flex items-center gap-2 px-4 py-2 rounded-full text-sm font-bold bg-white/5 text-brand-secondary border border-brand-secondary/30"
            >
              <Globe className="w-4 h-4" />
              {t("lang_toggle")}
            </button>
            {!loading && (user ? (
                <button onClick={() => { logout(); closeMobileMenu(); }} className="text-neutral-500 hover:text-red-400">
                  <LogOut className="w-5 h-5" />
                </button>
              ) : (
                <Link to="/auth" onClick={closeMobileMenu} className="text-brand-primary font-bold">
                  Acceder
                </Link>
              )
            )}
